...
void reduce_all_labels (int min_online_roi_size)
{
	...
	for (auto& ld : labelData) // for (auto& lv : labelUniqueIntensityValues)
	{
		...
		for (auto& pix : r.raw_pixels)
		{
			...
			if (!found)
			{
				Contour.push_back(pix);	// Register a contour pixel
		...
		//==== Perimeter
		r.perimeter = (StatsInt)Contour.size();
		...
		//==== Enclosing circle
		MinEnclosingCircle cir;
		r.diameter_min_enclosing_circle = cir.calculate_diam (Contour);
	}
