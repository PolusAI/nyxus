...
{
	// Materialize the ROI's pixels from the disk-backed cloud and reuse the identical in-RAM
	// calculate(). The previous bespoke streaming copy had drifted: it computed eccentricity from
	// major/minor instead of minor/major (a negative radicand -> NaN) and elongation as
	// sqrt(minor/major) instead of minor/major, so it disagreed with the in-RAM path on any
	// non-circular ROI. Delegating keeps trivial == out-of-core by construction.
