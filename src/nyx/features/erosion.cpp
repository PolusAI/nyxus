...
void ErosionPixelsFeature::osized_calculate (LR& r, const Fsettings& s, ImageLoader&)
{
	...
	// Reset numErosions first: save_value() runs after this early return, so leaving the member at a
	// previous oversized ROI's cap would report that stale count instead of the skipped-ROI's 0.
