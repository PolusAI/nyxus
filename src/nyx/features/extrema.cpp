#include "extrema.h"
	...
	ExtremaFeatures::ExtremaFeatures() {}

	void ExtremaFeatures::initialize (const std::vector<Pixel2>& roi_pixels)
	{
		int TopMostIndex = -1;
		int LowestIndex = -1;
		int LeftMostIndex = -1;
	...
	void ExtremaFeatures::reduce (size_t start, size_t end, std::vector<int>* ptrLabels, std::unordered_map <int, LR>* ptrLabelData)
	{
		for (auto i = start; i < end; i++)
		{
			...
			ExtremaFeatures extrema;
			extrema.initialize(r.raw_pixels);
			auto [x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7, x8, y8] = extrema.get_values();
