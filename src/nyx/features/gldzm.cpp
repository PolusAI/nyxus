...
void GLDZMFeature::calculate (LR& r)
{
	...
	using ACluster = std::pair<PixIntens, int>;	// <zone intensity, zone area>
	...
	// Squeeze the copy's intensity range for better performance
	...
	{
		// binned intensity
		unsigned int I_b = Nyxus::to_grayscale(D[i], 0, piRange, nGrays, Environment::ibsi_compliance);
		U.insert(I_b);
		D[i] = I_b;
	}
	...
	for (int row = 0; row < M.height; row++)
		for (int col = 0; col < M.width; col++)
		{
			...
			int zoneMetric = dist2closestRoiBorder (D, x, y);
			...
			for (;;)
			{
				...
				if (D.safe(_y,_x) && D.yx(_y,_x) != VISITED && D.yx(_y,_x) == inten)
				{
					...
					int dist2roi = dist2closestRoiBorder (D, _x, _y);
	...
	// total of zone distances in the image
	double Ns = 0;
	for (auto p : P)
		Ns += p;
	...
	calc_auxiliary_sums (Mx, Md, P, Ng, Nd);
	...
	{
		double m = Md[d-1];
		f_SDE += m / double(d*d);	// Small Distance Emphasis = \frac{1}{N_s} \sum_d \frac{m_d}{d^2}
	...
	f_LDLGLE /= (double)Ns;
	f_LDHGLE /= (double)Ns;
	f_ZP = (double)Ns / (double)r.aux_area;		// Zone Percentage = \frac{N_s}{N_v}
	f_GLE = f_ZDE;
...
int GLDZMFeature::dist2closestRoiBorder(const pixData& D, const int x, const int y)
{
	// scan left
	int dist2l = 0;
	for (int x0 = x - 1; x0 >= 0; x0--)
		if (D.yx(y, x0) == 0 || x0 == 0)	// we're at the ROI border or left margin (x0==0)
		{
			dist2l = x - x0;
			break;
		}
	// scan right
	int dist2r = 0, w = D.width();
	for (int x0 = x + 1; x0 < w; x0++)
		if (D.yx(y, x0) == 0 || x0 == w - 1)	// we're at the ROI border or right margin (x0==w-1)
		{
			dist2r = x0 - x;
			break;
		}
	...
	for (int y0 = y - 1; y0 >= 0; y0--)
		if (D.yx(y0, x) == 0 || y0 == 0)	// we're at the ROI border or top margin (y0==0)
		{
			dist2t = y - y0;
			break;
		}
	// scan down
	int dist2b = 0, h = D.height();
	for (int y0 = y + 1; y0 < h; y0++)
		if (D.yx(y0, x) == 0 || y0 == h - 1)	// we're at the ROI border or bottom margin (y0==h-1)
		{
			dist2b = y0 - y;
			break;
		}
...
void GLDZMFeature::calc_auxiliary_sums (std::vector<double> & Mx, std::vector<double>& Md, const SimpleMatrix<int>& P, const int Ng, const int Nd)
{
