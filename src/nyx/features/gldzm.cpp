...
void GLDZMFeature::osized_calculate (LR& r, const Fsettings& s, ImageLoader&)
{
	...
	// to_grayscale() instead of TextureFeature::bin_intensities(), so zones were grouped by
	// different grey levels and the distance-zone features disagreed with the trivial path.
