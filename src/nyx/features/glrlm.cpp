...
void GLRLMFeature::osized_calculate (LR& r, const Fsettings& s, ImageLoader&)
{
	...
	// to_grayscale() instead of TextureFeature::bin_intensities()), which ran off the end of a
	// run-length matrix sized to the trivial binning -- an out-of-bounds access that crashed -- and
	// produced values disagreeing with the trivial path. Delegating guarantees trivial == out-of-core.
