...
void GLSZMFeature::osized_calculate (LR& r, const Fsettings& s, ImageLoader&)
{
	...
	// to_grayscale() instead of TextureFeature::bin_intensities(), so the zone scan found no
	// matching grey levels and every feature came back 0 instead of the trivial path's value.
