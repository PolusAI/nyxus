...
void PixelIntensityFeatures::report_in_source_domain (const LR& r, const Dataset& ds)
{
	...
	// on 0, which is what trips the guards -- and perfectly ordinary in the slide's own domain,
...
void PixelIntensityFeatures::calculate_grey_levels (LR& r, const Fsettings & fsett, const Dataset & ds)
{
	...
	// map puts the slide's own minimum on grey level 0 -- and on an identity-mapped slide it is
...
void PixelIntensityFeatures::osized_calculate_grey_levels (LR& r, const Fsettings& stng, const Dataset& ds, ImageLoader& imloader)
{
	...
	double mad = 0.0,
		var = 0.0;
	// out-of-core: dispersion is accumulated from the disk-backed pixel cloud, not from raw_pixels
	// (which an oversized ROI never populates, so these came back 0 instead of the in-RAM values)
	for (size_t i = 0; i < r.raw_pixels_NT.size(); i++)
	{
		Pixel2 px = r.raw_pixels_NT[i];
		double diff = px.inten - mean_;
	...
	// Median absolute deviation
	double medad = 0.0;
	// out-of-core: read from the disk-backed pixel cloud (see the dispersion loop above)
	for (size_t i = 0; i < r.raw_pixels_NT.size(); i++)
	{
		Pixel2 px = r.raw_pixels_NT[i];
		medad += std::abs(px.inten - median_);
