...
// The 64-bit seek of each platform: fseek takes a long offset, which is 32 bits on Windows, and a
// cloud file passes 2 GB at about 134M voxels. Declared in the header so the offset it computes can
// be asserted without a 2 GB fixture.
void Nyxus::seek_record (FILE* f, size_t offset)
{
#ifdef _WIN32
	int rc = _fseeki64 (f, (__int64) offset, SEEK_SET);
#else
	int rc = fseeko (f, (off_t) offset, SEEK_SET);
#endif
	if (rc != 0)
		throw std::runtime_error ("ERROR in OutOfRamVoxelCloud - cannot seek to byte " + std::to_string (offset));
}
...
Pixel3 OutOfRamVoxelCloud::get_at (size_t idx) const
{
	...
	if (blk_first < 0 || idx < (size_t) blk_first || idx >= (size_t) blk_first + blk_count)
	{
		...
		Nyxus::seek_record (pF, start * item_size);
...
void OutOfRamVoxelCloud::read_range (size_t first, size_t count, std::vector<Pixel3>& out) const
{
	...
	Nyxus::seek_record (pF, first * item_size);
