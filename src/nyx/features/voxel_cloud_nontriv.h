...
namespace Nyxus
{
	/// @brief Seek 'f' to byte 'offset' through the 64-bit seek of the platform. A cloud file passes
	/// the 2 GB a 32-bit offset holds at about 134M voxels, which an oversized ROI reaches. Throws
	/// when the seek fails.
	void seek_record (FILE* f, size_t offset);
}
