...
namespace Nyxus
{
	...
	const char colname_intensity_image[] = "intensity_image",
		...
		colname_c_index[] = "c_index",		// channel index column, mirrors t_index
		colname_phys_unit[] = "phys_unit",	// physical-size unit (leading string column)
		colname_phys_x[] = "phys_x",		// physical voxel spacing columns (numeric)
	...
	// Why a loader cannot feed an out-of-core pass, and what to do about it: shared by the
	// segmented and whole-volume refusals so the two cannot drift apart. The caller prefixes what
	...
	int featurize_3d_wv_thread (Environment& env, const std::vector<std::string>& intensFiles,
		const std::vector<std::string>& labelFiles, size_t slide_idx, size_t nf,
		const std::string& outputPath, bool write_apache, Nyxus::SaveOption saveOption);
	// Featurizes one slide of the 2D whole-slide workflow, reporting its status through 'rv'.
	// Declared here so a test can drive it directly: its refusal of a slide whose loader will
	// not open is not reachable through processDataset_2D_wholeslide, whose prescan rejects
	// such a slide first.
	void featurize_wsi_thread (Environment& env, const std::vector<std::string>& intensFiles,
		const std::vector<std::string>& labelFiles, size_t slide_idx, size_t nf,
		const std::string& outputPath, bool write_apache, Nyxus::SaveOption saveOption, int& rv);
	...
	// mask with more voxels than the intensity. A refusal is raised under Python and printed
	// otherwise, and false is returned. 'mask_fpath' is empty in whole-volume mode.
	...
	// Opens an intensity-mask pair in env.theImLoader with the intensity domain the prescan recorded
	// for slide 'slide_idx' (its range and load-time map). A bare SlideProps carries neither, and a
	// pair opened with one maps its grey levels differently from the prescan and from every other
	// pass over the slide. An index that names no scanned slide opens the pair bare.
