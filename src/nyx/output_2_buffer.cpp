...
namespace Nyxus
{
		...
		const std::unordered_map<int, FmapChildInfo> & childToParentMap)
	{
		...
		if (fill_header)
		{
			rescache.add_to_header({ Nyxus::colname_intensity_image, Nyxus::colname_mask_image });
			rescache.add_to_header(std::string("parent_roi_label"));
			rescache.add_to_header(std::string("kernel_center_x"));
			rescache.add_to_header(std::string("kernel_center_y"));
			rescache.add_to_header(std::string(Nyxus::colname_roi_label));
			rescache.add_to_header(std::string(Nyxus::colname_t_index));
			write_feature_header_buffer(rescache, env, F);
		}
		...
		for (auto l : L)
		{
			...
			// String columns
			rescache.add_string(intens_name);
			rescache.add_string(seg_name);

			// Fmaps-specific numeric columns
			rescache.add_numeric(info.parent_label);
			rescache.add_numeric(info.center_x);
			rescache.add_numeric(info.center_y);

			// ROI label (child) and time index
			rescache.add_numeric(l);
			rescache.add_numeric(DEFAULT_T_INDEX);
		...
		const std::unordered_map<int, FmapChildInfo> & childToParentMap)
	{
		...
		size_t map_size = (size_t)map_w * map_h;
		...
		for (auto l : childLabels)
		{
			...
			if (map_row < 0 || map_row >= map_h || map_col < 0 || map_col >= map_w)
				continue;

			size_t pixel_idx = (size_t)map_row * map_w + map_col;

			// Collect feature values for this child
			auto vals = collect_feature_values(r, F);
			for (size_t fi = 0; fi < n_features && fi < vals.size(); fi++)
				feature_data[fi * map_size + pixel_idx] = force_finite_number(vals[fi], env.resultOptions.noval());
		}
