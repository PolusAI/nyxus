...
namespace Nyxus
{
		...
		size_t c_index)
	{
		...
		if (fill_header)
		{
			// phys_unit is a leading STRING column (kept with intensity/mask so the
		...
		// - slide info + phys_unit (3rd leading string column)
		...
		// this plane's own time and channel indices
	...
	bool save_features_2_buffer (ResultsCache& rescache, Environment & env, size_t t_index, size_t c_index)
	{
		...
		for (auto l : L)
		{
			...
			rescache.add_string (slide.phys_unit);		// 3rd leading string column
			rescache.add_numeric (l);
			rescache.add_numeric ((double)t_index);
			rescache.add_numeric ((double)c_index);		// channel index column, mirrors t_index
			rescache.add_numeric (slide.phys_x);		// physical voxel spacing (numeric)
