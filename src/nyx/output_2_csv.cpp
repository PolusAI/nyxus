...
#define fopen_s(pFile,filename,mode) ((*(pFile))=fopen((filename),(mode)))==NULL
	...
	std::vector<std::string> get_header (Environment & env) 
	{
		...
		// Physical-size unit (a leading STRING column so the Python buffer's
		...
		// channel (mirrors t_index; one column per channel plane)
		head.push_back (Nyxus::colname_c_index);

		// physical voxel spacing (numeric columns)
		...
		size_t c_index)
	{
		...
		// Create or continue, per PATH. This sink is called once per (channel, timeframe) plane
		// while separatecsv derives one path per slide, so the first write to a path truncates it
		// and renders the header and every later one appends -- which is what the t_index/c_index
		// columns are for, and what the Arrow and buffer sinks do. Keyed by path rather than by a
		// function-static flag, which could only express this for singlecsv.
		...
		if (! r.blacklisted)
		{
			...
			// slide info
			// physical calibration of this slide (phys_unit is a leading string column,
		...
		bool need_aggregation)
	{
		...
		// Create or continue, per PATH -- see the whole-slide sink above. The 3D segmented
		...
		if (need_aggregation)
		{
			...
			if (allres.size())
			{
				...
				const auto& v0 = std::get<FTABLE_FBEGIN>(tup0);		// the feature block starts at FTABLE_FBEGIN, after the index/calibration columns
				...
				for (const auto& tup : allres)
				{
					...
					const auto& v = std::get<FTABLE_FBEGIN>(tup);		// the feature block starts at FTABLE_FBEGIN
				...
				std::stringstream ssVals;
				// ... slide/mask file info + phys_unit (fnames[2] is the leading unit string)
				...
				// ... ROI id
				ssVals << "," << -1;
				// ... time and channel (the aggregate row carries them like every other row,
				// misaligning it against the header; emit both so columns line up)
				ssVals << "," << t_index << "," << c_index;
				// ... physical voxel spacing (numeric columns, from the 1st ROI's tuple)
		...
		else // no aggregation
		{
			...
			for (auto l : L)
			{
				...
				fs::path pseg (sli.fname_seg), 
					pint (sli.fname_int);
				// phys_unit is a leading string column (after the file names)
		...
		size_t c_index)
	{
		...
		// physical calibration of this slide
		const SlideProps& sp = env.dataset.dataset_props[r.slide_idx];
		...
		// the plane's own time/channel indices (the sentinel -999.88 stands for
		...
		size_t c_index)
	{
		...
		for (const auto l : L)
		{
			...
			filenames.push_back(sli.phys_unit);		// phys_unit leading string column
			...
			// the plane's own time/channel indices, in place of the
