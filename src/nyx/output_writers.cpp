...
arrow::Status ParquetWriter::setup(const std::vector<std::string>& header) {
    ...
    // schema is [intensity, mask, phys_unit] (utf8) + ROI_label (int32) + the rest
...
arrow::Status ParquetWriter::write (const std::vector<FTABLE_RECORD>& features) {
    ...
    //cccccccc intensity
    {
        ...
        //cccccccc phys_unit (3rd leading string column)
        b.Reset();
    ...
    //cccccccc channel (mirrors the time column so multi-channel input is addressable)
    {
        arrow::DoubleBuilder b;
    ...
    //cccccccc physical voxel spacing (phys_x/y/z numeric columns)
...
arrow::Status ArrowIPCWriter::setup(const std::vector<std::string>& header) {
    ...
    // [intensity, mask, phys_unit] (utf8) + ROI_label (int32) + float64 rest
    fields.push_back(arrow::field(header[0], arrow::utf8()));
    fields.push_back(arrow::field(header[1], arrow::utf8()));
    fields.push_back(arrow::field(header[2], arrow::utf8()));
    fields.push_back(arrow::field(header[3], arrow::int32()));
