...
namespace Nyxus
{
	...
	void feed_pixel_2_metrics(int x, int y, unsigned int z, PixIntens intensity, int label, unsigned int tile_index)
	{
		if (uniqueLabels.find(label) == uniqueLabels.end())
		{
			...
			LR newData;
			init_label_record_2(newData, theSegFname, theIntFname, x, y, z, label, intensity, tile_index);
		...
		else
		{
			...
			update_label_record_2(existingData, x, y, z, label, intensity, tile_index);
		}
	}
