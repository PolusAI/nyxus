...
namespace Nyxus
{
	...
	void init_label_record_hierarchical (HieLR& r, const std::string& segFile, const std::string& intFile, int x, int y, int label, PixIntens intensity, unsigned int tile_index)
	{
		r.init_aabb(x, y);
		...
		int x, int y, int label, unsigned int tile_index)
	{
		if (UL.find(label) == UL.end())
		{
			...
			HieLR newData;
			init_label_record_hierarchical (newData, theParFname, "no2ndfile", x, y, label, 999/*dummy intensity*/, tile_index);
			RD[label] = newData;
		}
		else
		{
...
bool gatherRoisMetrics_H(const std::string& fpath, std::unordered_set <int>& uniqueLabels, std::unordered_map <int, HieLR>& roiData)
{
	...
	// Open a mask image
	ImageLoader1x imlo;
	FpImageOptions fpopt;
	fpopt.set_target_dyn_range(1e4);	// some default range
	bool ok = imlo.open (fpath, fpopt);
	if (!ok)
	{
		std::stringstream ss;
...
//
// note: requires a cleared results buffer
//
