...
def write_tiled(name, order):
    ...
    tiled). tile=(16,16) makes the 6x8 plane a single tile. Exercises the tile-loader
    (z,c,t)->IFD path, distinct from the strip loaders."""
    path = os.path.join(HERE, name)
    data = _data_for(order)
...
def main():
    ...
    # single-channel tiled Z-stack whose 40x24 planes span a 3x2 grid of 16x16 tiles, the last
    # row-tile 8 tall and the last col-tile 8 wide -> multi-tile assembly and its seam clamp,
    # through both the facade and the prescan
    write_multitile("dim3_oddtile.ome.tif", T=1, C=1, Z=3, Y=40, X=24)
