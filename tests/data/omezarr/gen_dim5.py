...
Stores (same encoding throughout; absent axes are pinned to index 0):
  ...
  dim3_chunked.ome.zarr axes z,y,x      3D, chunk (3,4,5) -> uneven chunk grid in Z, Y and X
  dim5_zwholechunk.ome.zarr axes t,c,z,y,x  Z chunk = whole Z, T=2 -> one read is a whole frame

Unless its writer below says otherwise, a store is chunked one z-slice per chunk and written
UNCOMPRESSED (compressor=None) so it reads with header-only z5 (no blosc/zlib link).
...
def write_chunked(name, order, chunks):
    """Same coordinate encoding as write_store, but chunked by `chunks` (one extent per axis
    of `order`) instead of one z-slice per chunk. Real OME-Zarr chunks span several Z-planes
    and split each plane across a grid; a chunk extent that does not divide its axis leaves a
    PARTIAL last chunk. A reader that fetches only chunk (0,0) of a plane, or only the first
    Z-plane of a chunk, returns wrong data for the rest."""
    path = os.path.join(HERE, name)
    shutil.rmtree(path, ignore_errors=True)
    data = _data_for(order)
    ...
    ms = {"version": "0.4", "axes": [_AX[ax] for ax in order],
          "datasets": [{"path": "0",
                        "coordinateTransformations": [{"type": "scale", "scale": [1.0] * len(order)}]}]}
    g.attrs.put({"multiscales": [ms]})
    print("wrote %-24s axes=%-6s shape=%s chunks=%s" % (name, order, list(data.shape), list(chunks)))
...
def main():
    ...
    # chunk (3,4,5) over the 4x6x8 volume: Z-chunk depths 3,1, row-chunks 4,2, col-chunks 5,3
    write_chunked("dim3_chunked.ome.zarr", "zyx", (3, 4, 5))
    ...
    # Z chunk spanning the whole volume, with T=2: one read of this store is every Z-plane
    # of a frame, the case an out-of-core pass has nothing smaller to stream. T>1 is what
    # makes it discriminating -- the store reports its time extent through fullTimestamps
    # while its tile carries a single frame, so the two extents must be read separately.
    write_chunked("dim5_zwholechunk.ome.zarr", "tczyx", (1, 1, Z, Y, X))
