...
class TestIntensityHistogram:
    ...
    def test_2d_intensity_histogram_requires_ibsi_mechanics(self):
    ...
    def test_2d_intensity_histogram_enabled_with_ibsi_mechanics(self):
    ...
    def test_2d_intensity_histogram_values_integer_domain_analytic(self):
        nyx = nyxus.Nyxus(features=["*ALL_IH*"], ibsi=True, coarse_gray_depth=3)
        df = nyx.featurize(IH_INT, IH_SEG)
        assert df.shape[0] == 1
        row = df.iloc[0]
    ...
    def test_2d_intensity_histogram_index_features_within_bins_invariant(self):
        N = 3
...
class TestMergeRois:
    ...
    def test_2d_intensity_histogram_mergerois_per_label_default_mechanics(self):
        nyx = nyxus.Nyxus(features=["MEAN"])
        df = nyx.featurize(self.INT, self.SEG)
        assert df.shape[0] == 2   # one row per label

    def test_2d_intensity_histogram_mergerois_collapses_to_one_roi_mechanics(self):
        nyx = nyxus.Nyxus(features=["MEAN"], mergerois=True)
        df = nyx.featurize(self.INT, self.SEG)
        assert df.shape[0] == 1   # all foreground merged into a single ROI

    def test_2d_intensity_histogram_mergerois_excludes_background_mechanics(self):
