...
- test_ooc_2d_matches_in_ram_mechanics: the out-of-core path must produce the same intensity features
  as the in-RAM path on the same file pair.
- test_ooc_montage_oversized_fails_loudly_mechanics: the in-memory montage path has no out-of-core
...
def test_ooc_2d_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_glcm_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_glszm_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_gldzm_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_wholevolume_streams_oob_mechanics(tmp_path):
    """The WHOLE-VOLUME (single_roi) 3D path -- Nyxus3D.featurize_directory(dir, dir, ...), i.e.
    label_dir==intensity_dir -- streams an oversized whole volume through the same
    populate_3d_voxel_cloud/run_3d_ooc_features primitives as the segmented path. This is the
    Python-facing equivalent of the gtest TEST_3D_WHOLEVOLUME_OVERSIZED_STREAMS_OOC_MECHANICS: force
...
def test_ooc_3d_blank_matches_in_ram_mechanics(tmp_path):
...
def test_ooc_3d_nifti_unsupported_format_fails_loudly_mechanics():
    """NIfTI delivers the whole X*Y*Z*T volume in one read, so a tile layer of it IS the whole
    cube and streaming it cannot bound the footprint out-of-core exists to bound. An oversized ROI
    through this loader must fail loudly with an actionable message rather than allocate the cube
    and OOM, crash, hang, or silently emit a wrong/zero row. ram_limit=0 forces every ROI oversized
    regardless of its actual footprint (roiFootprint >= 0 is always true)."""
    ...
    # the message names the condition (how many planes one read delivers, of which input), not
    # the format: a chunked OME-Zarr whose Z chunk spans the volume is refused by the same check.
    # compat_int_mri is 32 planes deep in one time frame, and this loader hands back all of them
    # in a single read, so the count is what the reader has to hold at once.
    with pytest.raises(Exception, match="a single read of the intensity input delivers 32 planes"):
        n.featurize_files([intp], [segp], False)


def test_ooc_3d_partial_mask_matches_in_ram_mechanics(tmp_path):
...
def _make_volume_pair_negative(dtype):
    def make(tmp_path):
        # The field of _make_volume_pair shifted to straddle zero, in a signed or real-valued type.
        # The loader offsets such a slide by the minimum the prescan measured, so every pass must
        # be opened with the domain the prescan recorded; a pass opened with a bare SlideProps
        # offsets by 0 and clamps the negative half to grey level 0.
        Z, Y, X = 8, 90, 90
        z = np.arange(Z)[:, None, None]
        y = np.arange(Y)[None, :, None]
        x = np.arange(X)[None, None, :]
        inten = (x + y * 90 + z * 900).astype(np.float64) - 3000.0
        if np.dtype(dtype).kind == "f":
            inten = inten + 0.25
        inten = np.broadcast_to(inten, (Z, Y, X)).astype(dtype)
        mask = np.ones((Z, Y, X), np.uint32)
        intp = tmp_path / ("vol_int_neg_%s.ome.tif" % np.dtype(dtype).name)
        segp = tmp_path / ("vol_seg_neg_%s.ome.tif" % np.dtype(dtype).name)
        tifffile.imwrite(str(intp), inten, metadata={"axes": "ZYX"})
        tifffile.imwrite(str(segp), mask, metadata={"axes": "ZYX"})
        return str(intp), str(segp)
    return make
...
def test_ooc_3d_negative_intensity_matches_in_ram_mechanics(tmp_path, dtype):
    """An oversized 3D ROI of a signed or real-valued volume holding negative intensities must be
    mapped like the in-RAM ROIs: the out-of-core pass opens the file pair with the intensity
    domain the prescan recorded, so its features match the in-RAM path's."""
    _ooc_vs_ram_3d(tmp_path, ["*3D_ALL_INTENSITY*", "*3D_GLCM*"], pair_fn=_make_volume_pair_negative(dtype))
