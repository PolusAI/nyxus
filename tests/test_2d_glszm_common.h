...
// Shared fixture for the 2D GLSZM tests: the settings recipes and the IBSI-phantom scaffolding that
...
// Fsettings, glszm.h for GLSZMFeature, test_data.h for the phantom arrays and test_main_nyxus.h for
...
// The two configurations the 2D GLSZM tests run on. IBSI mode at 128 grey levels is the
// discretisation that makes the published consensus values and a mirp run comparable; Nyxus' default
// mode weights the grey-level-dependent features by raw intensity instead of the grey-level index,
// which no reference reproduces, so it is drift-pinned rather than vetted.
static Fsettings make_glszm2d_settings (bool ibsi_mode, int grey_depth)
{
    Fsettings s;
...
static std::vector<double> glszm_2d_phantom_slice_values (const Feature2D& feature_, const Fsettings& s)
{
