...
// Given the neighbor graph (which CellProfiler vets, see test_2d_neighbor_cellprofiler.h),
	...
	const std::string& feature_name)
{
	...
	ASSERT_TRUE(neighbor_2d_analytic_ref_vals_by_label.at(label).count(feature_name) > 0) << feature_name;
	ASSERT_NEAR(roiData.at(label).fvals[static_cast<int>(feature)][0],
		neighbor_2d_analytic_ref_vals_by_label.at(label).at(feature_name), 1e-4);
}
...
void test_2d_neighbor_percent_touching_enclosed_analytic()
{
	std::unordered_map<int, LR> roiData;
	calculate_neighbor_feature_values(roiData);
