...
#include "test_glcm_pyradiomics.h"
...
//
// ORACLE-VETTED GLCM keys (PyRadiomics) live in test_glcm_pyradiomics.h
// (oracle_pyradiomics_glcm_feature_golden_values). This file keeps UNVETTED snapshots only.
...
{
	if (glcm_pyradiomics_has(golden_key)) {
		found = true;
		return glcm_pyradiomics_golden(golden_key);
	}
