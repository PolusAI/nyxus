...
// (oracle_coverage.csv, target_test=test_morphology_cellprofiler.h): the 5 EDGE_* edge-intensity
// statistics and MASS_DISPLACEMENT. They were asserted inside two mixed-kind functions in
// test_morphology_regression.h; SPEC 2 keeps one kind per file, so they are split out here.
//
// PROVENANCE RECORD MISSING (SPEC 6.4): the values come from the shared golden map in
// test_morphology_common.h, which is still named for a snapshot and carries no CellProfiler version,
...
void test_morphology_edge_intensity_cellprofiler()
{
