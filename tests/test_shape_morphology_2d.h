...
static std::unordered_map<std::string, double> unvetted_nyxus_regression_shape2d_feature_golden_values{
	...
	// FRACT_DIM_PERIMETER: closed-contour Richardson divider (was an out-of-range 0.319). No
	// software implements the divider method, so this is a regression value; the method is
	// oracle-validated on the Koch snowflake (D=log4/log3) in tests/python/test_fractal_dim_oracle.py.
	{"FRACT_DIM_PERIMETER", 1.2632300929080915},
};
...
static std::unordered_map<std::string, double> oracle_3p_shape2d_feature_golden_values{
	...
	// Box-counting dimension, reproduces the ImageJ/FracLac shifting-grid box counter to full
	// precision on this ROI (== log2(3)); the method also recovers analytic shapes exactly
	// (square->2, Sierpinski->1.585) in tests/python/test_fractal_dim_oracle.py.
	{"FRACT_DIM_BOXCOUNT", 1.5849625007211565},
...
void test_shape2d_verifiable_with_3p_builtin_oracle_fractal_circle_features()
{
	...
	// Box-count matches the ImageJ/FracLac shifting-grid oracle; perimeter (divider) has no
	// software oracle, so it is a regression value (method oracle-validated in the python suite).
