...
reference table declared as a raw `std::unordered_map` rather than through an alias. That last one
is what keeps the set closed: a table identified only by the words in its name is one rename away
from being invisible to the check, whereas `ref_vals_map<double>` says what it is regardless. One value type, `double`, for the same
