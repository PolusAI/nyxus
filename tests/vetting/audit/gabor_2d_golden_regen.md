...
`tests/test_dsb2018_data.h`, so the C++ tests and the oracle share one copy of the pixels. Each ROI
is the whole image (the gtest feeds every pixel under one label), so no mask is applied.
...
recipe, pairs = g.CONFIGS["gabor_2d_skimage_documented_defaults_ref_vals"]
...
- **Kernel normalization order.** Nyxus L1-normalizes its full kernel; the oracle L1-normalizes the
  ...
  f0 = pi/4, 21% at pi/2, 31% at 3pi/4, 48% at f0 = 4, and 100% from f0 >= 16 (generator part D).
- **Convolution and border.** `mode="full"` then cropped at `offset = ceil(n/2)`, which is what
  ...
  an untruncated kernel and is *not* interchangeable here: scoring off it moves values by up to 1.0
  (report §4.1). If you regenerate through anything other than `gabor_kernel` + crop + zero-padded
  `full` convolution, you are pinning a different feature.
