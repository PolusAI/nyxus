...
**`POWER_SPECTRUM_SLOPE` is pinned twice, and neither pin comes from a tool.** On the 8x12 fixture the
algorithm does not run at all: `rps()` returns early unless `floor(min(h,w)/8) >= 3` and the ROI is
8 px wide, so the pinned 0 is the guard's return value and its band is `abs=0`. The second pin,
`POWER_SPECTRUM_SLOPE_LARGE_ROI = 1.7837481542489078`, is Nyxus' own output on the 24x24 ROI built in
`test_imq_regression.h` — regenerate it by running that gtest case and reading the value out of the
failure message, the same way any snapshot here moves. Vetting the feature, as opposed to guarding
it, still needs the radial-binning defect fixed first — see `matrix/imq.md`.

**The four `SATURATION_*` cell pins** are exact fractions of pixel counts on ROIs built in the same
file (`k/16`), so they are derived by counting rather than regenerated by a tool.

**`SHARPNESS` has a candidate oracle that has been measured and refuted.** The reference is the
GPL-3.0 pydom package, installed rather than copied into this tree — re-run the comparison with

```
conda activate nyxus_mirp
pip install git+https://github.com/umang-singhal/pydom.git    # once; audit env only
