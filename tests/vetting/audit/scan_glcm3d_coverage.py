"""Regenerate glcm_3d_coverage.csv by scanning the 2D morphology tests. Stdlib only.
...
OUT = os.path.join(HERE, "glcm_3d_coverage.csv")
REGISTRY = os.path.join(VETTING, "oracle_coverage.csv")

# test_3d_glcm_coverage.h instantiates two parameterized suites over the family's featureset, so
...
def scan(path, feat_re):
    ...
    # 3D tests name features by enum (Feature3D::GLCM_ACOR_AVE) while the registry carries the
    # leading dimension digit (3GLCM_ACOR_AVE). Normalise so one pattern matches both spellings.
    # The equivalence tests alias the enum (`using F = Nyxus::Feature3D;`), so cover `F::` too.
    text = text.replace("Feature3D::", "Feature3D::3")
    text = re.sub(r"\bF::(?=GLCM_)", "F::3", text)
    hits = {}
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_3d_glcm_\w+)\s*\(\s*\)", fh.read()))
