...
OUT = os.path.join(HERE, "gldzm_2d_coverage.csv")
...
# (assert_gldzm_feature_mirp, assert_gldzm_feature_ibsi, ...), so the call line that names the
# feature is `assert_gldzm_feature_<oracle>(Feature2D::X, "X")`, not a bare ASSERT_ macro.
...
def feature_names():
    """The family's feature names, longest first so GLDZM_SDLGLE wins over GLDZM_SDE."""
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "2D" and r["family"] == "gldzm"]
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_2d_gldzm_\w+)\s*\(\s*\)", fh.read()))
