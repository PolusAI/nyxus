...
OUT = os.path.join(HERE, "glszm_2d_coverage.csv")
...
# every GLSZM assertion names its feature on the assertion line. Kept so the mechanism is here if a
...
# (assert_glszm_feature_mirp, assert_glszm_feature_ibsi, ...), so the call line that names the
# feature is `assert_glszm_feature_<oracle>(Feature2D::X, "X")`, not a bare ASSERT_ macro.
...
def feature_names():
    """The family's feature names, longest first so GLSZM_SALGLE wins over GLSZM_SAE."""
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "2D" and r["family"] == "glszm"]
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_2d_glszm_\w+)\s*\(\s*\)", fh.read()))
