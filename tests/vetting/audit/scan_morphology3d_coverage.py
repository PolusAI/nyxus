...
def scan(path, feat_re):
    ...
    # 3D tests name features by enum (Feature3D::MORPHOLOGY_SRE_AVE) while the registry carries the
    # leading dimension digit (3MORPHOLOGY_SRE_AVE). Normalise so one pattern matches both spellings.
    # The _AVE test aliases the enum (`using F = Nyxus::Feature3D;`), so cover `F::` too.
    text = text.replace("Feature3D::", "Feature3D::3")
    text = re.sub(r"\bF::(?=MORPHOLOGY_)", "F::3", text)
    hits = {}
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "3D" and r["family"] == "morphology"]
...
def render(rows, asserted, oracles, regression, other):
    ...
    for r in rows:
        ...
        # duplication legible: test_3d_morphology_compatibility asserts the same _AVE features against the
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_3d_morphology_\w+)\s*\(\s*\)", fh.read()))
