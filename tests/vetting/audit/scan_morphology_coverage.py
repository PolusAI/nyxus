...
Deliberately NOT counted: a line that merely READS a feature out of the buffer. The 2D morphology
tests contain the case that makes this matter -- test_2d_morphology_documented_formula_conformance_analytic
reads AREA_PIXELS_COUNT, PERIMETER and MAJOR_AXIS_LENGTH to build the CIRCULARITY and ROUNDNESS
formulas, and asserts none of the three. A readout-counts-as-coverage rule credits an oracle test
with vetting three features it never checks; report_feature_tests.py does exactly that, which is why
its morphology gap count (17) is two rows short of the real one (19).
...
OUT = os.path.join(HERE, "morphology_2d_coverage.csv")
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "2D" and r["family"] == "morphology"]
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_2d_morphology_\w+)\s*\(\s*\)", fh.read()))
