"""Regenerate neighbor_2d_coverage.csv by scanning the 2D neighbour tests. Stdlib only.
...
oracle-suffixed function contributes an oracle token. That is exactly why the PERCENT_TOUCHING bound
assertions had to leave test_2d_neighbor_analytic.h: under an _analytic suffix this scanner would
credit the feature with an oracle the registry correctly says it does not have.
"""
import argparse
import csv
import io
...
OUT = os.path.join(HERE, "neighbor_2d_coverage.csv")
...
# Each oracle file loops its five labels calling a per-oracle helper that names the feature on the
...
# (assert_neighbor2d_cellprofiler, assert_neighbor2d_analytic, ...), so the call line that names the
# feature is `assert_neighbor2d_<kind>(roiData, label, Feature2D::X, "X")`, not a bare ASSERT_ macro.
...
def feature_names():
    """The family's feature names, longest first so CLOSEST_NEIGHBOR1_DIST wins over its prefix."""
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "2D" and r["family"] == "neighbor"]
