"""Regenerate ngldm_2d_coverage.csv by scanning the 2D morphology tests. Stdlib only.
...
OUT = os.path.join(HERE, "ngldm_2d_coverage.csv")
...
def registry_rows():
    with open(REGISTRY, newline="", encoding="utf-8") as fh:
        return [r for r in csv.DictReader(fh)
                if r["dim"] == "2D" and r["family"] == "ngldm"]
...
def unregistered_tests(where):
    ...
    with open(os.path.join(TESTS, "test_all.cc"), encoding="utf-8", errors="replace") as fh:
        registered = set(re.findall(r"(test_2d_ngldm_\w+)\s*\(\s*\)", fh.read()))
