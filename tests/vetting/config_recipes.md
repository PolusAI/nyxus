...
- All 19 features are snapshotted here; 16 of them are separately `vetted` against MIRP at
  `ngldm3d.mirp_samelevels` below, which is the recipe that shares a grey-level ladder with the tool.
  The band here is `rel=1e-9`, tighter than that oracle's, because a snapshot of this build's own
  output need not survive another platform's float. See `audit/ngldm_3d_mirp_vetting_report.md`.
