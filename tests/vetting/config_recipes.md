...
- **Only three features are snapshotted here**, not the family's nineteen: `3NGLDM_DCP`,
  `3NGLDM_GLM` and `3NGLDM_DCM`, the ones no tool can judge. The other 16 are `vetted` against MIRP
  at `ngldm3d.mirp_samelevels` and `ngldm3d.mirp_ibsi_rawlevels` below, on this same fixture and
  config, so a snapshot of them here would pin the same Nyxus run to a second literal and add no path
  or config coverage. The band is `rel=1e-9`, tighter than the oracles', because a snapshot of this
  build's own output need not survive another platform's float. See
...
- The same phantom through MIRP 2.6.0: `by_slice=False`,
  ...
  difference level (alpha) 0 — the IBSI NGLDM coarseness. Generator: `oracles/gen_ngldm3d_mirp.py`.
- **Not config-matched, and referenced rather than asserted against.** MIRP spreads the ROI over
  levels 1-64; Nyxus bins it over `[0, ROI max]` of a volume shifted to a zero minimum and lands on
  levels 21-64. The comparison therefore measures the level ladder. `3NGLDM_GLM` and `3NGLDM_DCM`
  have no MIRP counterpart at all.

## ngldm3d.mirp_samelevels
- The same phantom and tool with `base_discretisation_method="none"`, MIRP handed the grey levels
  Nyxus' `GREYDEPTH=64` binning produces (`by_slice=False`, distance 1, alpha 0 as above). Both sides
  then compute the NGLDM over identical levels. Generator: `oracles/gen_ngldm3d_mirp.py`, which
  reproduces the Nyxus binning in `nyxus_grey_levels()`.
- **Config-matched, and where the family's oracle rows sit.** 16 features are `vetted` here in
  `test_3d_ngldm_mirp.h` at `rel=1e-3` (SPEC 7 same-definition tier; worst measured residual
  8.7e-16). Scope, per SPEC 4: the levels are reproduced from Nyxus' binning rather than measured out
  of Nyxus, so these assertions establish the NGLD matrix and the feature formulas over a given
  discretisation, not the discretisation itself. `3NGLDM_DCP` is excluded as a non-discriminating
  agreement, `3NGLDM_GLM` and `3NGLDM_DCM` because MIRP does not compute them.

## ngldm3d.mirp_ibsi_rawlevels
- The same phantom and tool at the family's **second config point**: Nyxus `IBSI=true`, which reaches
  `to_grayscale` as `disable_binning` and so does not bin at all — the raw loader-shifted intensity is
  the grey level, 2001 distinct of them on this fixture. MIRP runs
  `base_discretisation_method="none"` over the same raw values (`by_slice=False`, distance 1,
  alpha 0). Generator: `oracles/gen_ngldm3d_mirp.py`.
- **Config-matched by construction and asserted.** 16 features are `vetted` here in
  `test_3d_ngldm_mirp.h` at `rel=1e-3` (worst measured residual 7.54e-15). Unlike
  `ngldm3d.mirp_samelevels` there is no binning step reproduced in numpy for MIRP, because there is
  no binning — so these rows carry no narrowed scope.
- **A weak discriminator, measured:** 83.46% of ROI voxels have no matching neighbour at raw
  resolution and the maximum dependence reached is 17 of a possible 26. Asserted regardless, because
  SPEC §5.1 maps a VALID cell to an oracle assertion and this point exercises the neighbourhood and
  the ROI masking on a 2001-wide ladder rather than a 44-wide one. `3NGLDM_DCP` is excluded here for
  the same reason as at the other recipes: MIRP returns `ngl_dc_perc` = 1.0 at this config too.
...
- **`3GLSZM_ZE` is banded at `rel=1e-6` here, not at the exact tier.** A snapshot of the program's own
  arithmetic reproduces to the last bit on the platform that produced it, and these pins were taken on
  one. `fast_log10()` evaluates `fexp + a*signif*signif + b*signif` in `float`, which clang contracts
  into fused multiply-adds on arm64 and MSVC does not on x86-64, so the family's only logarithm lands
  in a different last bits there. Measured on this same phantom, `3GLRLM_RE` moves by **1.06e-9**
  between macOS-arm64 and Windows while every non-logarithm feature moves by ~1e-16 — which is why
  `test_3d_glrlm_regression.h` carries the identical carve-out, at the same `1e6`.
...
- The compat phantom (`compat_int/compat_int_mri.nii` + `compat_seg/compat_seg_liver.nii`, label 1) at
  `GREYDEPTH=100`, `IBSI=false`, `GLDM_GREYDEPTH=-20` (negative activates radiomics binCount-based
  binning, i.e. binCount 20). Oracle: `pyradiomics` (`binCount: 20`, `interpolator: sitkBSpline`,
  `resampledPixelSpacing:` empty, `weightingNorm:` empty, `imageType: Original`). Used by:
  `test_3d_gldm_pyradiomics.h`, `test_nyxus.py::test_3d_gldm_compatibility`. Generator:
  `oracles/gen_gldm3d_pyradiomics.py`.
- `GREYDEPTH=100` is inert here and recorded only because the settings vector carries it:
  `D3_GLDM_feature::calculate()` reads `GLDM_GREYDEPTH` and `IBSI` and nothing else.
- **There is no angle axis.** Unlike GLCM and GLRLM this family sums over the whole 26-voxel
  neighbourhood at once, so PyRadiomics' public value is the quantity Nyxus stores and there is no
  per-direction table to intercept and no `calc_ave` aggregation in play.
- The two sides are comparable without a convention allowance: PyRadiomics' defaults `distances=[1]`
  and `gldm_a=0` are what Nyxus' fixed 26-offset `shifts[]` table and its `pi == neig_pi` test
  implement, and both count the centre voxel toward its own dependence, so `j` means the same thing in
  the same column on both sides. That is why SPEC 7's exact tier applies, at `abs=1e-9`.
- `3GLDM_DE` is banded separately at `abs=4e-3` for `fast_log10()`; see the registry note and
  `audit/gldm_3d_pyradiomics_vetting_report.md`.

## gldm3d.regression_ut_phantom
- The segmented phantom (`phantoms/ut_inten.nii` + `phantoms/ut_mask57.nii`, label 57) at
  `GREYDEPTH=64`, `IBSI=false`, `GLDM_GREYDEPTH=64`. **No oracle** - Nyxus' own values, pinned as
  drift guards in `test_3d_gldm_regression.h` and regenerated by `test_3d_gldm_dump_regression()`.
- `GLDM_GREYDEPTH`'s **sign** is what separates this recipe from the PyRadiomics one: positive selects
  MATLAB-style level-count binning, negative selects PyRadiomics-style bin counting, and 0 selects the
  IBSI no-binning path. So the two recipes are different quantities on different phantoms, not two
...
## gldm3d.regression_ut_phantom_nobinning
- The same phantom, label and `GREYDEPTH=64` as the recipe above, with `GLDM_GREYDEPTH=0` and
  `IBSI=false`. **No oracle** - Nyxus' own values, pinned in `test_3d_gldm_regression.h` and
  regenerated by `test_3d_gldm_dump_nobinning_regression()`.
- This is the **compiled default**, not a third configuration someone might choose:
  `Environment::compile_feature_settings()` names no default for `GLDM_GREYDEPTH`, so the zero-filled
  settings vector leaves it at 0, and only `--3gldm/greydepth=N` moves it. Every run that does not
  pass that option computes the family here. `IBSI=true` reaches the same code path by forcing
  `greyInfo = 0`.
- It is a different quantity from the recipe above, not a rounding of it: on the same ROI
  `3GLDM_HGLE` is 4275550.79 against 1957.19 and `3GLDM_SDE` 0.86996 against 0.15361, so the `64`
  pins guard nothing here. `GREYDEPTH` is inert for this family and is held at 64 only so the two
  recipes differ in the one setting under test.
- **No oracle claim, and the oracle question is open**: whether PyRadiomics can be made to skip
  discretisation on a non-integer intensity volume has not been answered, so this stays a drift
  guard. `matrix/gldm3d.md` carries what would move the cell to VALID.

## gldm3d.regression_constant_roi
- A synthetic 4x4x3 ROI, every voxel intensity 7, fed to `D3_GLDM_feature::calculate()` directly at
  `GREYDEPTH=64`, `IBSI=false`, `GLDM_GREYDEPTH=0` - the compiled default - and a no-value sentinel of
  `-98765.0`. Benchmark `bench_constant_roi_3d`. **No oracle claim** - the pin is Nyxus' current
  output. The default is what this cell is asserted at because it is where the intercept below is the
  only thing between the ROI and a matrix.
- Reachable in production wherever a segmentation lands on a flat region. All fourteen come back as
  the sentinel (production default `0.0`, i.e. `--noval`) through `calculate()`'s `aux_min ==
  aux_max` intercept, whose comment says blank ROIs. At radiomics binning a second branch reaches the
  same sentinel independently -- `to_grayscale_radiomix()`'s `binW = (max - min) / binCount` is zero,
  so every voxel bins to background and `Nz == 0` -- which is the other reason this recipe is stated
  at `GLDM_GREYDEPTH=0`.
- **PyRadiomics 3.0.1 computes a full GLDM on the same voxels** at `binWidth=1, gldm_a=0,
  distances=[1]`: `P_gldm` is 1x4, `[8, 20, 16, 4]` over one grey level. Bypassing the intercept as a
  negative control, Nyxus reproduces it on all seven dependence-axis features to the last bit
