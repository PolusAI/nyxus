...
- Every 3D NGLDM feature sits here, which is unique among the 3D families. What keeps them here is
  the discretisation rather than the NGLDM: at `ngldm3d.mirp_samelevels` below Nyxus and MIRP agree
  on all seventeen comparable features to 8.7e-16, while at MIRP's own binning the two tools put this
  ROI on different level ladders. See `audit/ngldm_3d_mirp_vetting_report.md`.
...
- **Config-matched, agreeing to 8.7e-16 over the seventeen comparable features, not asserted
  against.** The levels are reproduced from Nyxus' binning rather than measured out of Nyxus, so an
  assertion here would establish the NGLD matrix and the feature formulas over a given
  discretisation, not the discretisation itself.
