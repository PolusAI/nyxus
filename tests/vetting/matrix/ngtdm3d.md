...
| any | any | ≥2 | **VALID in principle, not exercised** | the setting is honoured (`--3ngtdm/radius`), but no reference tool in the matrix exposes a distance other than 1 for NGTDM, so nothing could be compared against |
...
- **radius ≥ 2**: needs a tool exposing the NGTDM distance. PyRadiomics' `distances` setting does
  accept it — `distances=[2]` is a supported kernel — so this is reachable with the generator already
  in place, and is the cheapest unexercised cell to close.
