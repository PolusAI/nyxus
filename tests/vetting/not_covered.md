...
## A. Test files no registry row references — 31 files, 105 test functions
...
### A.1 Correctly absent — plumbing, fixtures and framework self-tests (24 files)
...
| `test_2d_intensity_degenerate_roi_mechanics.h` | 3 | the intensity ratios — `QCOD`, `UNIFORMITY_PIU` and `COV` — on a populated ROI of one grey level, read before the output NaN substitution, and the empty-ROI guard |
...
**`test_2d_morphology_cellprofiler.h`'s six assertions are closed by running the tool, and one of
them did not survive it.** Two grades of gap sat in this section and they were never equally
serious: for the MATLAB rows the values *are* the oracle's and only the reproduction recipe is
absent, whereas here even that much was unestablished — nothing in the tree showed the numbers had
ever come from CellProfiler, so the entry recorded an unproven claim rather than an unreproducible
one. That is what made it worth running first. `gen_morphology_cellprofiler.py` drives CellProfiler
4.2.8 `MeasureObjectIntensity` on the shape2d fixture; five of the six reproduce to 5.2e-8 relative
(the residual is CellProfiler's float32 image storage, nothing else) and now carry tool, version,
module, config recipe and generator. The sixth, `EDGE_STDDEV_INTENSITY`, does **not** reproduce and
is demoted to `regression`. What is left in the table above is the MATLAB kind of gap only.
...
is the point. Full detail in `matrix/imq.md`. The three cells that used to sit here and are now
pinned - `POWER_SPECTRUM_SLOPE` past the size guard, and `SaturationFeature` on a constant ROI and
on a narrow mask - moved out of this table and into `test_imq_regression.h`; a
VALID-BUT-PRODUCTION-ONLY cell belongs in a snapshot, not in a list of things nothing reaches.

**The out-of-core rows below stay open on purpose.** This pass vets the in-RAM paths. Guarding the
out-of-core ones per feature needs an oversized-ROI harness this family's fixture does not build,
and every entry here is a source defect, so they are carried together by the follow-up scoped at the
end of `matrix/imq.md` rather than half-closed by relabelling.

| site | what is uncovered | why it matters |
|---|---|---|
| `SaturationFeature::get_percent_max_pixels_NT()` | the whole out-of-core path | it uses two independent `if`s where the in-RAM `get_percent_max_pixels()` uses `else if`, so the two paths disagree by construction on a constant ROI - and on that ROI `osized_calculate()` returns before either runs, while the base's `osized_scan_whole_image()` calls `save_value()` anyway. Feature methods are singletons nothing resets between ROIs, so what is published is the previous oversized ROI's saturations, or an indeterminate double on the first. One input, three answers |
| `FocusScoreFeature::get_focus_score_NT()` | the whole out-of-core path | passes `(width, height)` where `laplacian()`'s first size parameter is the row count - the other branch passes `(winY, winX)` correctly; takes the variance over a `conv_buffer` larger than the region any pixel writes; in the small-ROI branch fills a 900-entry window buffer with `W[row*width + col]` over the full ROI, so a 100x20 ROI writes 2000 entries into 900; and in the large-ROI branch steps `n_winHor` windows sideways per window (`winHor * n_winHor * winX`) and assumes `width == n_winHor * winX`. Its `tile_variance`, commented "abs sum of tile", is declared and never used |
| `PowerSpectrumFeature::osized_calculate()`, `SharpnessFeature::osized_calculate()` | the whole out-of-core path | both are empty `{}` bodies overriding the base's pure virtual, and `FeatureMethod::osized_scan_whole_image()` calls the no-op and then `save_value()` regardless. No member has a default initializer and no constructor assigns one, so an oversized ROI publishes `POWER_SPECTRUM_SLOPE` and `SHARPNESS` from indeterminate doubles |
| `FocusScoreFeature::osized_calculate()`, `LOCAL_FOCUS_SCORE` | the whole out-of-core path | it assigns `focus_score_` only; `local_focus_score_` is assigned inside `calculate()`, on the in-RAM path alone, while `save_value()` writes both members either way. Same case as the two empty bodies above, so it is three features and not two |
| `PowerSpectrumFeature::rps()` radial binning | the *correctness* of the axis, not the code path | the path is now pinned on a 24x24 ROI (`test_imq_power_spectrum_slope_large_roi_regression`), so it runs - but the pin is a drift guard, not a vetting claim. It bins by `floor(sqrt(fft coefficient))+1` rather than by the frequency radius `sqrt(kx^2+ky^2)`, and indexes `raw_radii` from a loop bounded by the padded FFT size (1024 against 24); on the pinned fixture the largest index reached is 3, so the read stays in range there and nowhere else is it bounded. Vetting needs the rewrite plus `centrosome.radial_power_spectrum.rps` |
...
| `SaturationFeature` on a constant ROI, and on a mask narrower than the bounding box | the *agreement*, not the code path | both cells are now pinned (`test_imq_{min,max}_saturation_{constant_roi,narrow_mask}_regression`), so drift is caught. What stays uncovered is correctness: CellProfiler computes a different quantity on each - 100% for both extrema on a constant ROI, and mask-only counting on a narrow mask - so neither cell can carry an oracle claim |
