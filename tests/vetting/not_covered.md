...
| `test_3d_morphology_matlab.h` | 3 | `morphology_3d_matlab_ref_vals` (MATLAB R2025b regionprops3) records tool, properties, fixture and recipe, but no generator — the numbers cannot be regenerated from the tree. Bands are measured rather than assumed (`3VOXEL_VOLUME` 2.3e-04%, the two hull volumes 3.88%), so the gap here is reproducibility only |
