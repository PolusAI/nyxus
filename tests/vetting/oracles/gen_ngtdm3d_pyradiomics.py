...
def pyradiomics_ngtdm(img, msk, label):
...
def cross_table_checks(txt):
    ...
    """
    rows = [(lev, n, p, s) for lev, n, p, s in parse_matrix_pins(
        txt, "ngtdm_3d_pyradiomics_matrix_ref_vals")]
    pins = parse_scalar_pins(txt, "ngtdm_3d_pyradiomics_ref_vals")
    derived = reference_features(rows)
    print("\n# cross-table: the %d feature pins recomputed from the matrix pins" % len(pins))
    bad = 0
    for name in sorted(pins):
        bad = compare("%s from the matrix table" % name, derived[name], pins[name], bad)
    return bad
...
def range_checks(txt):
    ...
    for name, v in sorted(parse_scalar_pins(txt, "ngtdm_3d_pyradiomics_ref_vals").items()):
        if not v > 0:                      # all five are sums of non-negative terms over a
            print("  FAIL %s: %r is not > 0" % (name, v))   # non-degenerate ROI
            bad += 1
    for table, nvp in (("ngtdm_3d_pyradiomics_matrix_ref_vals", 48),
                       ("ngtdm_3d_pyradiomics_docmatrix_ref_vals", 16)):
...
def main():
    ...
    f, rows = pyradiomics_ngtdm(img, msk, LABEL)
    ref_rows = reference_ngtdm(levels, roi)

    print("# pyradiomics %s, SimpleITK %s, binWidth=%d, label=%d, distances=[1]"
          % (radiomics.__version__, sitk.__version__, BINWIDTH, LABEL))

    # the two references must agree before either is pinned
    if [r[:2] for r in rows] != [r[:2] for r in ref_rows]:
        print("  FAIL the two NGTDM references disagree on the levels/counts:\n"
              "       pyradiomics %s\n       reference   %s"
              % ([r[:2] for r in rows], [r[:2] for r in ref_rows]))
        return 1
    worst = max(max(rel(a[2], b[2]), rel(a[3], b[3])) for a, b in zip(rows, ref_rows))
    print("# pyradiomics vs the independent reference NGTDM: worst rel %.3g over %d levels"
          % (worst, len(rows)))
    ...
    print("# pyradiomics vs the independent reference features: worst rel %.3g" % worstf)
    ...
    _, doc_pyrad = pyradiomics_ngtdm(doc_img, doc_msk, 1)
    ...
    print("\n# paste-ready goldens: ngtdm_3d_pyradiomics_ref_vals")
    for name in sorted(feats):
        print(('\t{"%s", %r},' % (name, feats[name])).ljust(56)
              + "// original_ngtdm_%s" % PYRAD[name])

    print("\n# paste-ready goldens: ngtdm_3d_pyradiomics_matrix_ref_vals   { i, n_i, p_i, s_i }")
    for lev, n, p, s in rows:
    ...
    pins = parse_scalar_pins(txt_h, "ngtdm_3d_pyradiomics_ref_vals")
    print("\n# verifying %d pinned feature goldens against this run" % len(pins))
    for name, want in sorted(pins.items()):
        if name not in feats:
            print("  FAIL %s: pinned but PyRadiomics produces no such feature" % name)
            bad += 1
            continue
        bad = compare(name, feats[name], want, bad)
    for name in sorted(feats):
        if name not in pins:
            print("  FAIL %s: PyRadiomics produces it but the header pins nothing for it" % name)
            bad += 1

    for table, produced in (("ngtdm_3d_pyradiomics_matrix_ref_vals", rows),
                            ("ngtdm_3d_pyradiomics_docmatrix_ref_vals", doc_pyrad)):
