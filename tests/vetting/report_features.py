"""Generate report_output.csv and report_output.md -- the vetting registry joined to what the tree asserts.
...
OUT_CSV = os.path.join(HERE, "report_output.csv")
OUT_MD = os.path.join(HERE, "report_output.md")
...
COLUMNS = [
    ...
    "scan_n_oracle", "scan_n_regression", "scan_notes",
...
# Family x dim combinations in the registry with no scanner behind them, each with the reason. Every
# family has one today. Listed rather than silently skipped: a family nothing reads out of the tree
# is a hole in the report, and naming it is what keeps it from reading as coverage.
KNOWN_UNSCANNED = {}
...
ALLOWLIST = {
    ("2D", f, "claim-without-assertion"): (
        "promoted by the Octave/MATLAB harness, which is deliberately outside the test tree, so no "
        "in-tree assertion carries the matlab token (source=tracker); the tree asserts it as a "
        "regression drift guard")
    for f in ("P01", "P25", "P75", "P99", "QCOD", "ROBUST_MEAN")
}
...
def build(cov, rows):
    ...
    for r in rows:
        ...
        out.append({
            ...
            "scan_notes": scanlib.note_for(s.fam, f, c) if s else "",
...
def render_md(recs):
    ...
    if offenders:
        ...
        if any(r["verdict"] == "unscanned" for r in recs):
            lines += ["`unscanned` rows are not listed here; they are a property of the family, not",
                      "of the row, and are counted in the section below.", ""]
        lines += ["| dim | feature | verdict | claim | tree | allowed because |",
                  "|---|---|---|---|---|---|"]
    ...
    if un:
        ...
        lines += ["", "## Families with no scanner", "",
                  ...
                  f"cannot be checked against the tree at all.** No `audit/scan_*_coverage.py`",
                  "reads these families out of the test sources, so their rows record what someone",
                  "believed rather than what the tree asserts. Writing the missing scanners is what",
                  "closes this.", "",
...
def main(argv=None):
    ...
    ap.add_argument("--write", action="store_true", help="regenerate report_output.csv and report_output.md")
    ...
    if a.check:
        ...
        for key in sorted({(r["dim"], r["family"]) for r in recs if r["verdict"] == "unscanned"}):
            if key not in KNOWN_UNSCANNED:
                problems.append(f"{key[0]} {key[1]}: no scanner reads it out of the tree, "
